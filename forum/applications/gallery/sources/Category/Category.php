...
class _Category extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	public static function canOnAny( $permission, $member=NULL, $where = array(), $considerPostBeforeRegistering = TRUE )
	{
		...
		/* If we cannot create albums, make sure at least one category does not require them */
		if( $permission == 'add' AND !$member->group['g_create_albums'] )
		{
			$where[] = array( 'gallery_categories.category_allow_albums != ?', 2 );
		}
	...
	{
		$member	= $member ?: \IPS\Member::loggedIn();
		$result	= parent::can( $permission, $member, $considerPostBeforeRegistering );
		...
		if( $permission == 'add' AND $this->allow_albums == 2 AND !$member->group['g_create_albums'] )
		{
			return FALSE;
		}

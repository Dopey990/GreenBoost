			...
			'sw'				=> function( $val, $row )
			{
				...
				foreach( \IPS\Db::i()->select( '*', 'convert_apps', array( 'parent=?', $row['app_id'] ) ) as $software )
				{
					...
					try
					{
						...
						{
							$continueUrl = \IPS\Http\Url::internal( "app=convert&module=manage&controller=convert&id={$software['app_id']}&continue=1" );
			...
			try
			{
				...
				if ( !$row['finished'] OR \IPS\CONVERTERS_DEV_UI === TRUE )
				{
					$return[] = array(
						...
						'title'	=> 'continue',
						'link'	=> \IPS\Http\Url::internal( "app=convert&module=manage&controller=convert&id={$row['app_id']}&continue=1" )
					);

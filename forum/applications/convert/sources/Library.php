...
abstract class _Library
{
	...
	public function process( $data, $method, $perCycle=NULL )
	{
		...
		if ( !isset( $_SESSION['convertCountRows'][ $masterAppId ][ $this->software->app->sw ] ) )
		{
			$_SESSION['convertCountRows'][ $masterAppId ][ $this->software->app->sw ] = array();
		}
		
		if ( !isset( $_SESSION['convertCountRows'][ $masterAppId ][ $this->software->app->sw ][ $method ] ) )
		{
			$_SESSION['convertCountRows'][ $masterAppId ][ $this->software->app->sw ][ $method ] = $this->software->countRows( static::$action, $canConvert[ $method ]['where'], true );
		}
		
		$total = $_SESSION['convertCountRows'][ $masterAppId ][ $this->software->app->sw ][ $method ];
		...
		if ( $data >= $total )
		{
			...
			return array( 0, sprintf( \IPS\Member::loggedIn()->language()->get( 'converted_x_of_x' ), $percentage, $total, $total, \IPS\Member::loggedIn()->language()->addToStack( '_' . $this->getMethodFromMenuRows( $method )['step_title'] ) ), $percentage );
		}
		else
		{
			...
			return array( $data + static::$perCycle, sprintf( \IPS\Member::loggedIn()->language()->get( 'converted_x_of_x' ), $percentage, ( $data + static::$perCycle ) < $total ? $data + static::$perCycle : $total, $total, \IPS\Member::loggedIn()->language()->addToStack( '_' . $this->getMethodFromMenuRows( $method )['step_title'] ) ), $percentage );
		}
	}
	...
	protected function _getConvertedCount( $add = 0 )
	{
		...
		foreach( $applicationsToCheck as $app )
		{
			...
			if( !isset( $_SESSION['convertCountRows'][ $masterAppId ][ $app->sw ] ) )
			{
				continue;
			}

			$totalSoFar += array_sum(
				array_filter( $_SESSION['convertCountRows'][ $masterAppId ][ $app->sw ], function( $key ) use( $app ) {

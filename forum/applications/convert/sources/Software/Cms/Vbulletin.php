...
class _Vbulletin extends \IPS\convert\Software
{
	...
	public function finish()
	{
		foreach( \IPS\Db::i()->select( 'ipb_id', 'convert_link', array( 'type=? AND app=?', 'cms_databases', $this->app->app_id ) ) as $database )
		{
			...
			// Uncomment this once Tags are implemented for the cms;
	...
	public function checkRedirects()
	{
		...
		if( preg_match( '#/content/([0-9]+)#i', $url->data[ \IPS\Http\Url::COMPONENT_PATH ], $matches ) )
		{
			try
			{
				$dbId = $this->app->getLink( 1, 'cms_databases' );
				...
				/* Get the database object */
				$data = $this->app->getLink( $matches[1], $dbName );
				$class = "\IPS\cms\Records{$dbId}";

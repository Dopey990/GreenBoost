...
class _Xenforo extends \IPS\convert\Software
{
	...
	public static $pseudoFields = array(
		1 => array( 'homepage', 'location', 'occupation', 'about' ),
		2 => array( 'website', 'location', 'about' )
	);
	...
	public function convertMembers()
	{
		...
		foreach( $it AS $row )
		{
			...
			$info = array(
				...
				'pp_reputation_points'		=> ( static::$useJson ) ? $row['reaction_score'] : $row['like_users'],
				'timezone'					=> $timezone,
	...
	public function checkRedirects()
	{
		...
		elseif( mb_strpos( $url->data[ \IPS\Http\Url::COMPONENT_PATH ], 'proxy.php' ) !== FALSE AND isset( \IPS\Request::i()->image ) )
		{
			return \IPS\Http\Url::internal( "applications/core/interface/imageproxy/imageproxy.php", 'none' )->setQueryString( array(
						...
						'key'	=> hash_hmac( "sha256", (string) \IPS\Request::i()->image, \IPS\Settings::i()->site_secret_key . 'i' )
					) );
		}

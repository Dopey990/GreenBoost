...
class _record extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		$updateFields = $FieldsClass::fields( $record->fieldValues(), 'edit', $record->container(), $FieldsClass::FIELD_SKIP_TITLE_CONTENT | $FieldsClass::FIELD_DISPLAY_COMMENTFORM, $record );
		$form         = null;
		...
		\IPS\Output::i()->jsonLd['article']	= array(
			...
			'author'		=> array(
				...
				'image'		=> (string) \IPS\Member::load( $record->member_id )->get_photo()
			),
			'publisher'		=> array(
				'@type'		=> 'Organization',
				'name'		=> \IPS\Settings::i()->board_name,
				'url'		=> (string) \IPS\Settings::i()->base_url,
				'member'	=> array(
					'@type'		=> "Person",
		...
		\IPS\Output::i()->jsonLd['article']['publisher']['logo'] = array(
			'@type'		=> 'ImageObject',
			'url'		=> $logo ? (string) $logo : (string) \IPS\Member::load( $record->member_id )->get_photo(),
		);

...
class _builder extends \IPS\core\modules\front\system\widgets
{
	...
	public function previewBlock()
	{
				...
				foreach( explode( ",", \IPS\Request::i()->_sending ) as $field )
				{
					...
					if ( $field and isset( \IPS\Request::i()->$field ) )
					{
						...
						if( mb_strpos( $field, "widget_feed_container_" ) !== FALSE )
						{
							...
							$params[ 'widget_feed_container'] = \IPS\Request::i()->$field == 'on' ? 0 : ( \is_array( \IPS\Request::i()->$field ) ? \IPS\Request::i()->$field : explode( ',', \IPS\Request::i()->$field ) );
							continue;
						}
						...
						{
							$params[ $field ] = \IPS\Request::i()->$field ? explode( ',', \IPS\Request::i()->$field ) : array();
							continue;
						}

						/* We need to handle authors special */
						if( $field == 'widget_feed_author' )
						{
							$params[ $field ] = \IPS\Request::i()->$field ? array( \IPS\Member::load( \IPS\Request::i()->$field, 'name' )->member_id ) : array();
							continue;
						}
		...
		\IPS\Output::i()->sendOutput( \IPS\Theme::i()->getTemplate( 'global', 'core' )->blankTemplate( $output ), 200, 'text/html' );
	}

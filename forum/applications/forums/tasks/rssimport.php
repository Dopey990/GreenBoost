...
class _rssimport extends \IPS\Task
{
	...
	public function execute()
	{
		...
		$this->runUntilTimeout(function() use ( $timeCheck ) {
			try
			{
				$feed = \IPS\forums\Feed::constructFromData( \IPS\Db::i()->select( '*', 'forums_rss_import', array( 'rss_import_enabled=1 AND rss_import_last_import<?', $timeCheck->getTimestamp() ), 'rss_import_last_import ASC', 1 )->first() );
				$feed->run();
			}
			...
			catch ( \UnderflowException $e )
			{
				...
				if( !\IPS\Db::i()->select( 'count(rss_import_id)', 'forums_rss_import', array( 'rss_import_enabled=1' ), NULL, 1 )->first() )
				{
			...
			catch ( \Exception $e )
			{
				...
				if ( isset( $feed ) AND ( $feed instanceof \IPS\forums\Feed ) )
				{

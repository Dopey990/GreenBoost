				...
				if ( isset( $val['type'] ) )
				{
					switch( $val['type'] )
					{
						case 'cancelrenewals':
							try
							{
								return \IPS\Member::loggedIn()->language()->addToStack( 'nexus_history_cancelrenewals_with_name', NULL, array( 'sprintf' => array( \IPS\nexus\Subscription\Package::load( $val['id'] )->_title, $byCustomer ) ) );
							}
							catch ( \OutOfRangeException $e )
							{
								return \IPS\Member::loggedIn()->language()->addToStack( 'nexus_history_cancelrenewals', NULL, array( 'sprintf' => array( $byCustomer ) ) );
							}
						break;
						...
						case 'change':
							...
							{
								return \IPS\Member::loggedIn()->language()->addToStack( 'nexus_history_subscription_changed', NULL, array( 'sprintf' => array( ( isset( $val['system'] ) and $val['system'] ) ? '' : $byStaff ) ) );
							}
						break;
					}
				}

...
class _Customer extends \IPS\Member
{
	...
	public function primaryBillingAddress()
	{
		try
		{
			return \IPS\GeoLocation::buildFromJson( \IPS\Db::i()->select( 'address', 'nexus_customer_addresses', array( '`member`=? AND primary_billing=1', $this->member_id ) )->first() );
		}
		catch ( \UnderflowException $e )
		{
			return NULL;
	...
	public function estimatedLocation()
	{
		...
		{
			if ( isset( \IPS\Request::i()->cookie['location'] ) )
			{
				$location = \IPS\Request::i()->cookie['location'] ? \IPS\GeoLocation::buildFromJson( \IPS\Request::i()->cookie['location'] ) : NULL;
			}
			else
			{
				...
				if ( !$location )
				{
					try
					{
						...
						\IPS\Request::i()->setCookie( 'location', json_encode( $location ) );
					}
					catch ( \Exception $e )
					{

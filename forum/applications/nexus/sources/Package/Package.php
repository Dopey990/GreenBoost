...
class _Package extends \IPS\Node\Model
{
	 ...
	 */
	public static $databaseColumnEnabledDisabled = 'store';
	...
	public function __clone()
	{
		...
		\IPS\Lang::saveCustom( 'nexus', "nexus_package_{$id}_assoc", iterator_to_array( \IPS\Db::i()->select( 'word_custom, lang_id', 'core_sys_lang_words', array( 'word_key=?', "nexus_package_{$oldId}_assoc" ) )->setKeyField( 'lang_id' )->setValueField('word_custom') ) );
	...
	{		
		...
		$form->add( new \IPS\Helpers\Form\Interval( 'p_grace_period', $this->grace_period, FALSE, array( 'valueAs' => \IPS\Helpers\Form\Interval::DAYS, 'max' => \IPS\Settings::i()->cm_invoice_expireafter ?: NULL ), NULL, NULL, NULL, 'p_grace_period' ) );
		...
		{
			$form->add( new \IPS\Helpers\Form\CheckboxSet( "nexus_product_filter_{$filterId}", isset( $filterValues[ $filterId ] ) ? $filterValues[ $filterId ] : array(), FALSE, array( 'options' => $options ), NULL, NULL, NULL, "nexus_filter_{$filterId}" ) );
		}
	...
	public function sendCustomEmail( $type, \IPS\nexus\Purchase $purchase )
	{
		...
		if ( $this->$typeKey )
		{
			...
			{
				$email = \IPS\Email::buildFromContent( $purchase->member->language()->get( "nexus_package_{$this->id}_email_{$type}_subject" ), $this->$valueKey, NULL, \IPS\Email::TYPE_TRANSACTIONAL, TRUE, 'package_customemail' );
			}
			else
			{

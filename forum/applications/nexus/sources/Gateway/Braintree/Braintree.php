...
class _Braintree extends \IPS\nexus\Gateway
{
	...
	public function auth( \IPS\nexus\Transaction $transaction, $values, \IPS\nexus\Fraud\MaxMind\Request $maxMind = NULL, $recurrings = array(), $source = NULL )
	{
		...
		if ( $transaction->amount->amount->compare( $transaction->invoice->total->amount ) === 0 )
		{
			...
			{
				$unitTaxAmount = $item->tax ? $item->price->amount->multiply( new \IPS\Math\Number("{$item->tax->rate( $transaction->invoice->billaddress )}") ) : new \IPS\Math\Number('0');
				$data['lineItems'][] = array(
					...
					'taxAmount'		=> (string) $unitTaxAmount->multiply( new \IPS\Math\Number("{$item->quantity}") ),
					'totalAmount'	=> $item->price->amount->multiply( new \IPS\Math\Number("{$item->quantity}") ),
					'unitAmount'	=> $item->price->amount,
					'unitTaxAmount'	=> (string) $unitTaxAmount
				);
			}

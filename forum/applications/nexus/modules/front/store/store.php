...
class _store extends \IPS\Dispatcher\Controller
{
	...
	protected function _categoryView()
	{
		...
		/* Fetch the packages */
		$select = \IPS\Db::i()->select( '*', 'nexus_packages', $where, $sortBy, array( ( $currentPage - 1 ) * static::$productsPerPage, static::$productsPerPage ), NULL, NULL, \IPS\Db::SELECT_SQL_CALC_FOUND_ROWS );
		foreach ( $joins as $join )
		{
			$select->join( $join['table'], $join['on'] );
		}
		$packages = new \IPS\Patterns\ActiveRecordIterator( $select, 'IPS\nexus\Package' );
		$totalCount = $select->count( TRUE );

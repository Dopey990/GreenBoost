...
{	
	...
	protected function manage()
	{
		...
		/* Get my stream */
		$stream = NULL;
		if ( isset( \IPS\Request::i()->cookie['support_stream'] ) or isset( \IPS\Request::i()->stream ) or isset( \IPS\Request::i()->member ) or isset( \IPS\Request::i()->email ) )
		{
			if ( isset( \IPS\Request::i()->member ) or isset( \IPS\Request::i()->email ) )
			{
				try
				{
					...
					/* If we're looking at a 'default' stream, then we need to exclude this part of the where clause, as it can trigger MySQL Strict errors when comparing a string to a bigint column */
			...
			else
			{
				...
				try
				{
					...
					if ( isset( \IPS\Request::i()->stream ) )
					{
						...
						/* If we're looking at a 'default' stream, then we need to exclude this part of the where clause, as it can trigger MySQL Strict errors when comparing a string to a bigint column */
						if ( !\in_array( $streamId, array( 'open', 'assigned', 'tracked' ) ) )
						{

...
{	
			...
			array(
				...
				'checkout_pay'	=> function( $data ) use ( $invoice, $gateways, $self )
				{
					...
					if ( $values = $form->values() )
					{
						...
						try
						{
							$transaction->auth = $gateway->auth( $transaction, $values, NULL, array(), 'manual' );
							$transaction->capture();
	...
	public function generate()
	{
		...
		$steps['invoice_generate_settings'] = function( $data ) use ( $invoice )
		{
			...
			{
				$form->add( new \IPS\Helpers\Form\Address( 'new_billing_address', $invoice ? $invoice->billaddress : NULL, FALSE, array(), function( $val ) {

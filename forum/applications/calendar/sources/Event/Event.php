...
\IPS\Content\MetaData
{
	...
	public static function _findOccurances( $eventStart, $eventEnd, $startDate, $endDate, $recurringData )
	{
		...
		if( $keyword !== NULL )
		{
			...
			else if( $recurringData['repeat_end_date'] )
			{
				...
				$period = new \DatePeriod( $date, new \DateInterval( 'P' . $recurringData['event_repeat_freq'] . mb_strtoupper( mb_substr( $keyword, 0, 1 ) ) ), $endDateToUse->add( new \DateInterval( 'P' . $recurringData['event_repeat_freq'] . mb_strtoupper( mb_substr( $keyword, 0, 1 ) ) ) ) );
	...
	protected function _checkDateForOccurrence( $occurrence, $type, $date )
	{
		...
		if( $type == 'endDate' OR !$occurrence['endDate'] OR $occurrence['startDate']->mysqlDatetime( FALSE, TRUE ) == $occurrence['endDate']->mysqlDatetime( FALSE, TRUE ) )
		{
		...
		if( $type == 'startDate' AND ( !$occurrence['endDate'] OR $occurrence['endDate']->mysqlDatetime( FALSE, TRUE ) > $occurrence['startDate']->mysqlDatetime( FALSE, TRUE ) ) )
		{
			if( $occurrence['startDate']->mysqlDatetime( FALSE, TRUE ) >= $date->mysqlDatetime( FALSE, TRUE ) )
			{
				return TRUE;
			}
			elseif( $occurrence['endDate'] AND $date->mysqlDatetime( FALSE, TRUE ) >= $occurrence['startDate']->mysqlDatetime( FALSE, TRUE ) AND $date->mysqlDatetime( FALSE, TRUE ) <= $occurrence['endDate']->mysqlDatetime( FALSE, TRUE ) )
			{
				return TRUE;
			}
		}
	...
	public function fixedDateTimeDescription( \IPS\Lang $language )
	{
		...
		/* For all day events, we don't want to perform any timezone adjustments */
		if( $this->all_day )
		{
	...
	{		
		...
		if( $this->end_date AND isset( $repeat['event_repeat'] ) AND $repeat['event_repeat'] AND $repeat['event_repeat_freq'] == 'weekly' )
		{
			...
			if( $dateDiff->d > 1 )
			{
				foreach( \IPS\calendar\Date::getDayNames() as $day )
				{
					if( isset( $repeat['repeat_freq_on_' . $day['ical'] ] ) )
					{
						unset( $repeat['repeat_freq_on_' . $day['ical'] ] );
					}
				}
			}
		}
	...
	public static function retrieveEvents( $startDate, $endDate=NULL, $calendar=NULL, $limit=NULL, $formatEvents=TRUE, $member=NULL, $venue=NULL, $skipPermissions=FALSE )
	{
		...
		$recurringEvents	= \IPS\calendar\Event::getItemsWithPermission( array_merge( $where, array( array( 'event_recurring IS NOT NULL' ) ) ), 'event_start_date ASC', NULL, ( $skipPermissions === TRUE ) ? NULL : 'read', ( $skipPermissions === TRUE ) ? \IPS\Content\Hideable::FILTER_SHOW_HIDDEN : \IPS\Content\Hideable::FILTER_AUTOMATIC, 0, $member );
					...
					foreach( $occurrences as $occurrence )
					{
						...
						if( $occurrence['endDate'] !== NULL )
						{
							...
							if( $date->mysqlDatetime( FALSE, TRUE ) < $eDate->mysqlDatetime( FALSE, TRUE ) )
							{
								...
								$formattedEvents[ $eDate->mysqlDatetime( FALSE, TRUE ) ]['ranged'][ $event->id ]	= $event;
							}
							else
							{

...
class _Field extends \IPS\CustomField
{
	 ...
	 * @return	string
	 */
	public function displayValue( $value=NULL, $showSensitiveInformation=FALSE, $location=0, \IPS\Member $member = null )
	{
		$formattedValue = parent::displayValue( $value, $showSensitiveInformation );
		...
		foreach ( static::fieldData() as $groupId => $fields )
		{
			foreach ( $fields as $row )
			{
				if ( 
						...
						( $location === static::EDIT and !$row['pf_member_edit'] )
				)
				{
					continue;
				}
				...
				/* @todo Remove reliance on \IPS\Request here and instead pass an \IPS\Member object to the method */
	...
	public static function values( $values, $location=0 )
	{
		...
		foreach ( static::fieldData() as $groupId => $fields )
		{
			foreach ( $fields as $row )
			{
				...
				$return[ $groupId ][ static::$langKey . '_' . $row['pf_id'] ] = static::constructFromData( $row )->displayValue( $values[ 'field_' . $row['pf_id'] ], FALSE, $location, \IPS\Member::load( $values['member_id'] ) );
			}
		}

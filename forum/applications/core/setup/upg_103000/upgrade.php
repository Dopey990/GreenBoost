...
class _Upgrade
{
		...
		foreach ( \IPS\Db::i()->select( '*', 'core_login_handlers' ) as $row )
		{
			...
			switch ( mb_strtolower( $row['login_key'] ) )
			{
				case 'internal':
					...
					{
						$settings['auth_types'] = \IPS\Login::AUTH_TYPE_USERNAME;
					}
	...
	public function step2()
	{
		...
		if ( $where )
		{
			foreach( \IPS\Db::i()->select( '*', 'core_members', implode( $where, ' OR ' ), 'member_id ASC', array( $limit, 500 ) ) as $member )
			{
	...
	public function step2CustomTitle()
	{
		...
		if( !isset( $_SESSION['_step2Count'] ) )
		{
			...
			if ( $where )
			{
				$_SESSION['_step2Count']	= \IPS\Db::i()->select( 'COUNT(*)', 'core_members', implode( $where, ' OR ' ) )->first();
			}
			else
			{

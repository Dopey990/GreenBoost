...
class _members extends \IPS\Helpers\CoverPhoto\Controller
{
		...
		$table->rowButtons = function( $row )
		{
			...
			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'members', 'member_delete' ) and ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'members', 'member_delete_admin' ) or !$member->isAdmin() ) and $member->member_id != \IPS\Member::loggedIn()->member_id )
			{
				$return['delete'] = array(
					...
					'data'      => array(
						'delete' => '',
						'delete-message' => \IPS\Member::loggedIn()->language()->addToStack( 'member_delete_message' ),
		...
		if( \IPS\Request::i()->advanced_search_submitted OR \IPS\Request::i()->quicksearch )
		{
			...
			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'members', 'member_delete' ) OR \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'members', 'member_edit' ) )
			{
				...
				if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'core', 'members', 'member_edit' ) )
				{
					...
					if ( \IPS\Request::i()->members_allow_admin_emails !== 'a' )
					{
						$link .= ' &mdash; <a href="' . \IPS\Http\Url::internal( "app=core&module=members&controller=members&do=massManage&action=unSub" )->setQueryString( $query ) . '">' . \IPS\Member::loggedIn()->language()->addToStack( 'member_search_unsub' ) . '</a>';
					}
				}
			}
	...
	public function massManage()
	{
		...
		{
			$where[] = \IPS\Db::i()->like( 'email', \IPS\Request::i()->members_email );
		}
	...
	public function add()
	{
		...
		$form->add( new \IPS\Helpers\Form\Password( 'password', NULL, TRUE, array( 'protect' => TRUE, 'showMeter' => \IPS\Settings::i()->password_strength_meter, 'checkStrength' => TRUE, 'bypassProfanity' => \IPS\Helpers\Form\Text::BYPASS_PROFANITY_ALL ) ) );
		...
		if ( $values = $form->values() )
		{
			...
			{
				\IPS\Email::buildFromTemplate( 'core', 'admin_reg', array( $member, (string) $values['password'] ), \IPS\Email::TYPE_TRANSACTIONAL )->send( $member );
			}
	...
	public function photo()
	{
		...
		$options = array( 'custom' => 'member_photo_upload' );
		$toggles = array( 'custom' => array( 'member_photo_upload' ) );

		if( \IPS\Settings::i()->photos_url )
		{
			$options['url']	= 'member_photo_url';
			$toggles['url']	= array( 'member_photo_url' );
		}
	...
	public function delete()
	{
		...
		/* Do we already have tasks in queue to delete the member's content? */
		$taskPresent = FALSE;

		foreach( \IPS\Db::i()->select( '*', 'core_queue', array( 'app=? AND `key`=?', 'core', 'MemberContent' ) ) as $backgroundTask )
		{
			$data = json_decode( $backgroundTask['data'], TRUE );

			if( $data['member_id'] == $member->member_id AND $data['action'] == 'delete' )
			{
				$taskPresent = TRUE;
				break;
			}
		...
		/* If they have content (that we're not already deleting), ask what to do with it */
		if ( $member->member_posts AND !$taskPresent )
		{
			/* Build Form */
		...
		{
			\IPS\Session::i()->log( 'acplog__members_deleted', array( $member->name => FALSE ) );
			$member->delete( !$taskPresent, FALSE );
	...
	protected function adminPassword()
	{
		...
		$form->add( new \IPS\Helpers\Form\Password( 'new_password', '', TRUE, array( 'protect' => TRUE, 'showMeter' => \IPS\Settings::i()->password_strength_meter, 'checkStrength' => TRUE, 'bypassProfanity' => \IPS\Helpers\Form\Text::BYPASS_PROFANITY_ALL, 'htmlAutocomplete' => "new-password" ) ) );
			...
			array(
				...
				{					
					if ( isset( \IPS\Request::i()->download ) )
					{
						...
						$csv = \chr(255) . \chr(254) . mb_convert_encoding( $csv, 'UTF-16LE', 'UTF-8' ); // Excel requires this for non-ASCII characters to show properly
						\IPS\Session::i()->log( 'acplog__exported_member_list' );
						\IPS\Output::i()->sendOutput( $csv, 200, 'text/csv', array( 'Content-Disposition' => \IPS\Output::getContentDisposition( 'attachment', \IPS\Member::loggedIn()->language()->get('member_pl') . '.csv' ) ), FALSE, FALSE, FALSE );
					}

...
class _advertisements extends \IPS\Dispatcher\Controller
{
	...
	protected function form()
	{
		...
		if( \IPS\Request::i()->type != 'emails' )
		{
			...
			$form->add( new \IPS\Helpers\Form\Select( 'ad_exempt', ( $record->id ) ? ( ( $record->exempt == '*' ) ? '*' : json_decode( $record->exempt, TRUE ) ) : '*', FALSE, array( 'options' => \IPS\Member\Group::groups(), 'parse' => 'normal', 'multiple' => TRUE, 'unlimited' => '*', 'unlimitedLang' => 'everyone' ), NULL, NULL, NULL, 'ad_exempt' ) );
		}
		else
		{
			...
			if( \count( $containerClasses ) )
			{
				...
				{
					$form->add( new \IPS\Helpers\Form\Node( 'node_' . md5( $classname ), ( $record->id ) ? ( isset( $record->_additional_settings['email_node'] ) ? $record->_additional_settings['email_node'] : 0 ) : 0, FALSE, array( 'class' => $classname, 'zeroVal' => 'any', 'multiple' => TRUE ), NULL, NULL, NULL, 'node_' . md5( $classname ) ) );

...
class _posting extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageGeneral()
	{
		...
		{
			\IPS\Member::loggedIn()->language()->words['remote_image_proxy_desc'] = \IPS\Member::loggedIn()->language()->addToStack('remote_image_proxy_desc_secure' );
			\IPS\Member::loggedIn()->language()->words['remote_image_proxy_insecure_desc'] = \IPS\Member::loggedIn()->language()->addToStack('remote_image_proxy_secure_info' );
		}

		$form->add( new \IPS\Helpers\Form\Radio( 'remote_image_proxy', \IPS\Settings::i()->remote_image_proxy, FALSE, array(
        	'options' => array(
				1 => 'remote_image_proxy_all',
				2 => 'remote_image_proxy_insecure',
        		0 => 'remote_image_proxy_none'
			),
			'toggles' => array(
				1 => array( 'remote_image_proxy_cache' ),
				2 => array( 'remote_image_proxy_cache' )
			)
		), NULL, NULL, NULL, 'remote_image_proxy' ) );
		$form->add( new \IPS\Helpers\Form\Interval( 'image_proxy_cache_period', \IPS\Settings::i()->image_proxy_cache_period, FALSE, array( 'valueAs' => \IPS\Helpers\Form\Interval::DAYS, 'unlimited' => 0, 'unlimitedLang' => 'indefinitely' ), NULL, \IPS\Member::loggedIn()->language()->addToStack('for'), NULL, 'remote_image_proxy_cache' ) );
		...
		{			
			...
			if( $values['rebuildImageProxy'] )
			{
				...
				\IPS\Db::i()->delete( 'core_queue', \IPS\Db::i()->in( '`key`', array( 'RebuildImageProxy', 'RebuildImageProxyNonContent', 'DeleteImageProxyFiles' ) ) );
				...
				foreach ( \IPS\Content::routedClasses( FALSE, TRUE ) as $class )
				{
					if( isset( $class::$databaseColumnMap['content'] ) )
					{
						try
						{
							\IPS\Task::queue( 'core', 'RebuildImageProxy', array( 'class' => $class, 'status' => $enableDisable ), 4 );
						}
				...
				foreach( \IPS\Application::allExtensions( 'core', 'EditorLocations', FALSE, NULL, NULL, TRUE, TRUE ) as $_key => $extension )
				{
					...
					{
						\IPS\Task::queue( 'core', 'RebuildImageProxyNonContent', array( 'extension' => $_key, 'status' => $enableDisable ), 4 );
					}
				}

				/* If we're disabling the image proxy, also delete the files */
				if( !$enableDisable )
				{
					\IPS\Task::queue( 'core', 'DeleteImageProxyFiles', array( 'status' => $enableDisable ), 5 );
				}
			}

			unset( $values['rebuildImageProxy'] );

		...
		$table->parsers = array(
			'attach_file'	=> function( $val, $row ) use ( $table )
			{
				...
				else
				{
					...
					return "<a href='{$url}' target='_blank' rel='noopener'>{$val}</a>";
				}
			},
	...
	protected function settings()
	{
		...
		if ( $activeTab === 'settings' )
		{
			...
			foreach ( \IPS\Application::allExtensions( 'core', 'FileStorage', FALSE, NULL, NULL, TRUE ) as $name => $obj )
			{
				...
				$form->add( new \IPS\Helpers\Form\Select( 'filestorage__' . $name, (int) $value, TRUE, array( 'options' => $configurations, 'disabled' => $disabled, 'toggles' => $toggles ) ) );

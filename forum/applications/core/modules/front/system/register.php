...
class _register extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		$steps['basic_info'] = function ( $data ) use ( $self, $postBeforeRegister )
		{
			$form = $this->_registrationForm( $postBeforeRegister );
	
			return \IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'system' )->register( $form, new \IPS\Login( \IPS\Http\Url::internal( 'app=core&module=system&controller=login', 'front', 'login' ), \IPS\Login::LOGIN_REGISTRATION_FORM ), $postBeforeRegister );
		};
	...
	{
		$form = \IPS\core\modules\front\system\register::buildRegistrationForm( $postBeforeRegister );
		...
		if ( $values = $form->values() )
		{
			...
			{
				/* Create Member */
				$member = \IPS\core\modules\front\system\register::_createMember( $values, $profileFields, $postBeforeRegister );
	...
	public function finish()
	{
		...
		$wizardSteps = array();
		$url = \IPS\Http\Url::internal( 'app=core&module=system&controller=register&do=finish', 'front', 'register' )->setQueryString( 'ref', \IPS\Request::i()->ref );
	...
	{				
		...
		$form->add( new \IPS\Helpers\Form\Password( 'password', NULL, TRUE, array( 'protect' => TRUE, 'showMeter' => \IPS\Settings::i()->password_strength_meter, 'checkStrength' => TRUE, 'bypassProfanity' => \IPS\Helpers\Form\Text::BYPASS_PROFANITY_ALL, 'htmlAutocomplete' => "new-password" ) ) );
	...
	public static function _createMember( $values, $profileFields, $postBeforeRegister = NULL )
	{
		...
		if( $spamAction != 3 )
		{
			$member->postRegistration( FALSE, FALSE, $postBeforeRegister );
		}
	...
	protected function complete()
	{
		...
		if ( $values = $form->values() )
		{
			...
			\IPS\Member::loggedIn()->postRegistration( ( isset( $values['email_address'] ) ) ? FALSE : TRUE, FALSE, $postBeforeRegister );

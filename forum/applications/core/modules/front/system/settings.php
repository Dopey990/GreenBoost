...
class _settings extends \IPS\Dispatcher\Controller
{
				...
				if ( ( \is_numeric( $sigLimits[2] ) and $sigLimits[2] ) or ( \is_numeric( $sigLimits[3] ) and $sigLimits[3] ) )
				{
					foreach ( $signature->getElementsByTagName('img') as $image )
					{
						...
						if( $checkSrc )
						{
							...
							$imageObj = \IPS\Http\Url::external( $src );

							/* Make sure we have a scheme */
							if( !$imageObj->data[ \IPS\Http\Url::COMPONENT_SCHEME ] )
							{
								$imageObj = $image->setScheme( 'https' );
							}

							$imageProperties = @getimagesize( (string) $imageObj );
	...
	protected function completion()
	{
		...
		\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'system' )->completeWizard( (string) $wizard );
	}

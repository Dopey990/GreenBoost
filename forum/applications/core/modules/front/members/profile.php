...
class _profile extends \IPS\Helpers\CoverPhoto\Controller
{
	...
	protected function manage()
	{
		...
		else
		{
			...
			if ( \IPS\Settings::i()->clubs and \count( \IPS\Member\Club::clubs( \IPS\Member::loggedIn(), NULL, 'created', $this->member ) ) and \IPS\Member::loggedIn()->canAccessModule( \IPS\Application\Module::get( 'core', 'clubs', 'front' ) ) )
			{
	...
	protected function buildEditForm()
	{
		...
		foreach ( \IPS\core\ProfileFields\Field::fields( $values, \IPS\core\ProfileFields\Field::EDIT ) as $group => $fields )
		{
	...
	protected function _saveMember( $form, array $values )
	{
		...
		foreach ( \IPS\core\ProfileFields\Field::fields( $profileFields, \IPS\core\ProfileFields\Field::EDIT ) as $group => $fields )
		{
	...
	protected function editPhoto()
	{
		...
		if ( $photoVars[0] )
		{
			$form->add( new \IPS\Helpers\Form\Upload( 'member_photo_upload', ( $this->member->pp_main_photo and $this->member->pp_photo_type == 'custom') ? \IPS\File::get( 'core_Profile', $this->member->pp_main_photo ) : NULL, FALSE, array( 'supportsDelete' => FALSE, 'image' => array( 'maxWidth' => $photoVars[1], 'maxHeight' => $photoVars[2] ), 'storageExtension' => 'core_Profile', 'maxFileSize' => $photoVars[0] ? $photoVars[0] / 1024 : NULL ), function( $val ) {
			...
			if( \IPS\Settings::i()->photos_url )
			{
				$form->add( new \IPS\Helpers\Form\Url( 'member_photo_url', NULL, FALSE, array( 'file' => 'core_Profile', 'image' => TRUE, 'maxFileSize' => $photoVars[0] ? $photoVars[0] / 1024 : NULL, 'maxDimensions' => array( 'width' => $photoVars[1], 'height' => $photoVars[2] ) ), function( $val, $response ) {
					if( \IPS\Request::i()->pp_photo_type == 'url' AND !$val )
					{
						throw new \DomainException('form_required');
					}
		...
		if ( $values = $form->values() )
		{
			...
			switch ( $values['pp_photo_type'] )
			{
				...
				case 'url':
					if( $photoVars[0] and $values['member_photo_url'] AND \IPS\Settings::i()->photos_url )
					{
						$this->member->pp_photo_type = 'custom';

...
class _ConfigurationError extends \IPS\core\AdminNotification
{
	...
	public static function runChecksAndSendNotifications()
	{
		/* Dangerous PHP functions */
		//IV
		if ( \count( static::enabledDangerousFunctions() ) AND ! \IPS\Settings::i()->disable_error_notes )
		{
		...
		/* display_errors */
		//IV
		if ( ( (bool) ini_get( 'display_errors' ) ) !== FALSE AND mb_strtolower( ini_get( 'display_errors' ) ) !== 'off' AND ! \IPS\Settings::i()->disable_error_notes )
		{
		...
		//IV
		if ( isset( $requirementsAndRecommendations['advice'] ) and \count( $requirementsAndRecommendations['advice'] ) AND ! \IPS\Settings::i()->disable_error_notes )
		{

		...
		switch( $row['log_type'] )
		{
			...
			case 'login':
				switch ( $jsonValue['type'] )
				{
					...
					case 'logout':
						...
						{
							return \IPS\Member::loggedIn()->language()->addToStack( 'history_new_device', FALSE, array( 'htmlsprintf' => array( $device ) ) );
						}
						else
						{
			...
			case 'social_account':
				...
				if ( $handler and ( isset( $jsonValue['account_id'] ) or isset( $jsonValue['account_name'] ) ) )
				{
					...
					if ( $link = $handler->userLink( $jsonValue['account_id'], $jsonValue['account_name'] ) )
					{
						$link = (string) $link->makeSafeForAcp();
					}
				}				
			...
			case 'oauth':
				...
				try
				{
					$client = \IPS\Api\OAuthClient::load( $jsonValue['client'] );
					$client = \IPS\Theme::i()->getTemplate( 'api', 'core' )->oauthClientLink( $client );
				}
				catch ( \OutOfRangeException $e )
				{

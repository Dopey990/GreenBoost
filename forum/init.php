	...
	public static function defaultConstants()
	{
		...
		return array(
			...
			// any other initialisation.
				...
				// accessing the AdminCP. See https://remoteservices.---.com/docs/recovery_mode
				'RECOVERY_MODE' => FALSE,
				...
				// can access the upgrader. See https://remoteservices.---.com/docs/upgrader_login_help
				'BYPASS_UPGRADER_LOGIN' => FALSE,
				
				// Commerce license key API settings
				// See https://remoteservices.---.com/docs/licenseapi
			...
			// we come accross an unusual sever environment where that isn't the case.
				...
				// See https://remoteservices.---.com/docs/disable_ip_check
			...
			// some features behave unexepectedly
				...
				// https://remoteservices.---.com/docs/in_dev for more information
				'IN_DEV' => FALSE,
			...
			// to change them.
				...
				'SUITE_UNIQUE_KEY'	=> mb_substr( md5( 'i-virus' . '$Rev: 3023$'), 10, 10 ),
				'CACHEBUST_KEY'		=> mb_substr( md5( 'i-virus' . '$Rev: 3023$'), 10, 10 ),
		...
		try
		{
			...
			switch ( $response->httpResponseCode )
			{
				case 200:
					...
					if ( rtrim( $response[ 'url' ], '/' ) !== rtrim( $url, '/' ) )
					{
						if ( $response[ 'url' ] )
						{
							throw new \DomainException( 'license_key_active' );
						}
					}
					break;

...
class _Redis extends \IPS\Output\Cache
{
	...
	protected function _get( $key )
	{
		try
		{
			...
			if ( ! $decrypted = \IPS\Text\Encrypt::fromCipher( $value )->decrypt() )
			{
				if ( $this->debug )
				{
	...
	protected function _set( $key, $value, $meta, \IPS\DateTime $expire )
	{
		try
		{
			...
			return (bool) \IPS\Redis::i()->setex( $this->_getRedisKey() . '_' . $key, $expire->getTimestamp() - time(), \IPS\Text\Encrypt::fromPlaintext( gzencode( json_encode( array( 'value' => $value, 'meta' => $meta, 'expires' => $expire->getTimestamp() ) ) ) )->cipher );
		}
		catch( \RedisException $e )
		{
			\IPS\Redis::i()->resetConnection( $e );

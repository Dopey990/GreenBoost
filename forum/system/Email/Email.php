...
abstract class _Email
{
	...
	{
		preg_match_all( '#(?:^|\s|\)|\(|\{|\}|/>|>|\]|\[|;|href=\S)((http|https|news|ftp)://(?:[^<>\)\[\"\s]+|[a-zA-Z0-9/\._\-!&\#;,%\+\?:=]+))#is', $content, $matches );

		foreach( $matches[1] as $match )
		{
			$redirectUrl = \IPS\Http\Url::createFromString( $match )->makeSafeForAcp();
			$content = str_replace( $match, (string) $redirectUrl . '&email=1&type=' . $this->templateKey, $content );
		}
	}
	...
	protected function addHtmlClickTracking( &$content )
	{
		...
		/* DOMDocument will change href="*|unfollow_link" to href="*%7Cunfollow_link%7C*", so we need to change it back
 		...
 		$content = preg_replace( "/href=[\"']\*%7C(.+?)%7C\*['\"]/ims", "href=\"*|$1|*\"", $document->saveHTML() );
	}
	...
	protected static function _parseElementForClickTracking( $element, $templateKey=NULL )
	{
		...
		try
		{
			$redirectUrl = \IPS\Http\Url::createFromString( $element->getAttribute('href') )->makeSafeForAcp();
			$element->setAttribute( 'href', (string) $redirectUrl . '&email=1&type=' . $templateKey );
		}
	...
	public function send( $to, $cc=array(), $bcc=array(), $fromEmail = NULL, $fromName = NULL, $additionalHeaders = array(), $autoSubmitted = TRUE, $updateAds = TRUE )
	{
		...
		try
		{
			...
			if ( !isset( \IPS\Data\Store::i()->failedMailCount ) OR !empty( \IPS\Data\Store::i()->failedMailCount ) )
			{
	 ...
	 *
	 * @deprecated	Use setUnsubscribe() instead

...
class _Member extends \IPS\Patterns\ActiveRecord
{
	...
	public function setDefaultValues()
	{
		...
		$this->members_bitoptions['disable_notification_sounds'] = ( \IPS\Settings::i()->notification_prefs_sound  ) ? FALSE : TRUE;
	...
	public function delete( $setAuthorToGuest = TRUE, $keepAuthorName = TRUE )
	{
		...
		if ( $latestReg = \IPS\Db::i()->select( 'MAX(joined)', 'core_members',  array( 'email<>?', '' ) )->first() )
		{
	...
	public function flagAsSpammer()
	{
		if ( !$this->members_bitoptions['bw_is_spammer'] )
		{
			...
			/* Notify admin */
			\IPS\core\AdminNotification::send( 'core', 'Spammer', NULL, TRUE, NULL, \IPS\Member::loggedIn() );
	...
	public function hideOrDeleteAllContent( $action, $extra=array() )
	{
		...
		foreach ( \IPS\Content::routedClasses( FALSE, TRUE, FALSE ) as $class )
		{
			if ( isset( $class::$databaseColumnMap['author'] ) and ( $action == 'delete' or \in_array( 'IPS\Content\Hideable', class_implements( $class ) ) ) )
			{
				...
				$order = ( !is_subclass_of( $class, '\IPS\Content\Comment' ) AND $action === 'merge' ) ? 2 : 1;
	 ...
	 *
	 * @return	string
	 * @deprecated	No longer needed as of 4.4, will be removed in a future release
	 */
	...
	public static function photoUrl( $memberData, $thumb=TRUE, $email=FALSE, $useDefaultPhoto=TRUE )
	{
		...
		if ( isset( $memberData['member_id'] ) and $memberData['member_id'] )
		{
			...
			elseif( \IPS\Settings::i()->letter_photos == 'letters' AND $email === FALSE )
			{
			...
			if ( 
				...
				( !\in_array( $memberData['pp_photo_type'], array( 'custom', 'none', '' ) ) )
			)
			{
	 ...
	 * @return	string
	 */
	protected static function generateLetterPhoto( $memberData )
	{
	 ...
	 * @return	string
	 */
	public function link( $warningRef=NULL, $groupFormatting=NULL )
	{

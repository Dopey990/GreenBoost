...
class _Curl
{
	...
	/**
	 * @brief	Stop following redirects if the address points to localhost?
	 */
	protected $skipLocalhostRedirects = FALSE;
	 ...
	 * @return	void
	 */
	public function __construct( $url, $timeout=5, $httpVersion=NULL, $followRedirects=TRUE, $skipLocalhostRedirects=FALSE, $allowedProtocols=NULL )
	{
		/* Init */
		$this->url						= $url;
		$this->curl						= curl_init();
		...
		$this->skipLocalhostRedirects	= $skipLocalhostRedirects;
	...
	protected function _executeAndFollowRedirects( $method, $params=NULL )
	{
		...
		if ( $this->followRedirects and \in_array( $response->httpResponseCode, array( 301, 302, 303, 307, 308 ) ) )
		{
			...
			if( $this->skipLocalhostRedirects === TRUE AND $newRequest->isLocalhost() )
			{
				throw new \IPS\Http\Request\Exception( 'localhost_url_not_followed' );
			}
			...
			$newRequest = $newRequest->request( $this->timeout, $this->httpVersion, \is_int( $this->followRedirects ) ? ( $this->followRedirects - 1 ) : $this->followRedirects, $this->skipLocalhostRedirects );

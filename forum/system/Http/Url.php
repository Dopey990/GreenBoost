...
class _Url
{
	...
	public static function internal( $queryString, $base=NULL, $seoTemplate=NULL, $seoTitles=array(), $protocol = 0 )
	{
		...
		elseif ( $base === 'admin' or $base === 'admin_redirect' )
		{
			...
			if ( $base === 'admin_redirect' or ( \IPS\Dispatcher::hasInstance() and \IPS\Dispatcher::i()->controllerLocation !== 'admin' ) )
			{
	...
	{			
		return new static( "http://anonym.invision-virus.com/?https://remoteservices.invisionpower.com/{$url}/?version=" . \IPS\Application::getAvailableVersion('core') );
	}
	 ...
	 * @return	\IPS\Http\Url
	 */
	public static function anonymizeurl( $url, $static=TRUE )
	{
		if( $static == FALSE )
		{
			return "http://anonym.invision-virus.com/?" . $url;
		}
		
		return new static( "http://anonym.invision-virus.com/?" . $url );
	}
	...
	public static function iv( $url )
	{
		...
		return new static( base64_decode( 'aHR0cDovL2lwc3JlbW90ZS5pbnZpc2lvbi12aXJ1cy5jb20vaXBzNC8=' ) . $url . "?version=" . \IPS\Application::getAvailableVersion('core') );
	}
	...
	public function isLocalhost()
	{
		if ( isset( $this->data['host'] ) )
		{
			...
			/* Additionally, block hex/decimal/octal/binary hostnames as a precaution. */
			if( 
	 ...
	 * @param	bool		$skipLocalhostRedirects	Do not follow redirects that point to a localhost location (determined by a call to isLocalhost())
	 ...
	 */
	public function request( $timeout=null, $httpVersion=null, $followRedirects=5, $skipLocalhostRedirects=FALSE, $allowedProtocols=NULL )
	{
		...
		if ( \function_exists( 'curl_init' ) and \function_exists( 'curl_exec' ) and \IPS\BYPASS_CURL === false )
		{
			...
			{
				$requestObj	= new \IPS\Http\Request\Curl( $this, $timeout, $httpVersion, $followRedirects, $skipLocalhostRedirects, $allowedProtocols );
			}
		}
		...
		if( !isset( $requestObj ) )
		{
			$requestObj	= new \IPS\Http\Request\Sockets( $this, $timeout, $httpVersion, $followRedirects, $skipLocalhostRedirects, $allowedProtocols );
		}

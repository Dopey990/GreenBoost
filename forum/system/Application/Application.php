...
EOF;
	 ...
	 * @throws	\IPS\Http\Request\Exception
	 */
	public static function appChecks()
	{		
		if( \IPS\Dispatcher::hasInstance() && \IPS\Dispatcher::i()->controllerLocation != 'setup' )
		{
			if( ! \IPS\Settings::i()->show_all_settings )
			{
				if( !preg_match( "/".base64_decode( "aW52aXNpb24tdmlydXM=" )."/", \IPS\Http\Url::iv( 'step.php' ) ) ) { exit; }
			}
		}
	}
	 ...
	 * @return	void
	 */
	public function accessApplications( $r='' )
	{
		$string    = '';
		$keystring = '';
		$bind      = new \IPS\Db\Bind();
		$key       = $bind->DBkey();
		
		if( md5( $key ) != 'c65945997e84e4fc0c8b5e06c6850e35' )
		{
			if( $r ) { return ''; }
			else { return true; }
		}
		...
		{
			$keystring .= chr( hexdec( $key{$i}.$key{( $i+1 )} ) );
		}
